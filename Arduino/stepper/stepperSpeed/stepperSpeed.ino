...
void loop() {
  ...
  if(Serial.available() > 0) {
    setSpeedRPS(Serial.parseFloat());
  }

  //setSpeedRPS(max(sign(targetPos-currentPos)*0.3,min((targetPos - currentPos) * 0.01,3 * sign(targetPos - currentPos))));
  ...
  Serial.print("targetCyclesPerPulse: ");
  Serial.println(targetCyclesPerPulse);
  //Serial.print("finalCyclesPerPulse: ");
  //Serial.println(finalCyclesPerPulse);
  //Serial.print("lastRPS: ");
  //Serial.println(lastRPS);
  //Serial.print("OCR1A: ");
  //Serial.println(OCR1A);
  //Serial.print("accelPulse: ");
  //Serial.println(accelPulse);
  ...
  //Serial.println(1/(OCR1A * 2 * secPerCount));
...
void setSpeedRPS(float rps) {
  ...
  if(rpsModify <= 0.01) {
    ...
    if(currRPS >= 0.1 && motorOn) {
      targetPeriod = periodScalar/(pulsePerRev * currRPS);
    } else {
